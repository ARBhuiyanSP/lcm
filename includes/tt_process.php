<?php
	 ...
	 * *****************************************************************************
	 */  
	if (isset($_POST['tt_submit']) && !empty($_POST['tt_submit'])) {
		
		// check duplicate:
		$sl_no		= $_POST['sl_no'];
		$table		= 'tt';
		$where		= "sl_no='$sl_no'";
		if(isset($_POST['tt_update_submit']) && !empty($_POST['tt_update_submit'])){
			$notWhere   	=   "id!=".$_POST['tt_update_submit'];
